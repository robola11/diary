...
export const loginUser = async (email, password) => {
    try {
        const [rows] = await pool.query('SELECT * FROM imagetest WHERE email = ?', [email]);
        if (rows.length === 0) {
...
export const getUserFromToken = async (token) => {
    try {
        ...
        const [rows] = await pool.query('SELECT id, name, email, mobile, userType FROM imagetest WHERE id = ?', [decoded.id]);
