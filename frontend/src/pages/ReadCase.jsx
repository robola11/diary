  ...
  const fetchUserDetails = async () => {
    ...
    const [cases, setCases] = useState([]);
       const [page, setPage] = useState(1);
          ...
          useEffect(() => {
            axios.get(`http://localhost:3000/readCase/+${id}?page=${page}&limit=5`)
            .then(res => {
  ...
  return ( 
    <div className="">
      ...
      <div className="flex between">
                <div>
                    <span className='sub-text overlay-text' datatype='Cases'>Endorsements</span>
                ...
                <div className="cards-container gap-1">
                    {cases.map((u) =>(
            <div className="card"  key={u.id}>
            
              <div className="cards-content">
                 <p style={{textTransform:'uppercase'}}>{u.suitnumber}</p>
                <h3 style={{textTransform:'uppercase'}}>{u.parties.length > 50 ? (u.parties.slice(0,50) + " ... "):(
              u.parties
            )}
            </h3>
                <h5 style={{paddingBottom:'10px', fontStyle:'italic'}}>"{u.comments}"</h5>
                <hr className="hr" />
               <small style={{paddingTop:'5px'}}>{u.date}</small>
              </div>
              </div>
